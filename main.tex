...
\begin{document}
    ...
    *Str. 132-133 - kodi operacij za ukaz RST 28 in tabele podprogramov* ~\cite{strojnijezikzaprocesorz80}
    ...
    (nn), A)}) v zlogih, ki sledita št. strojnega ukaza, vsebuje pomnilniški
    naslov operanda ali skočne lokacije, kar omogoča prenašanje vrednosti iz registra
    v pomnilniško celico in obratno ter skok ali prenos podatkov med poljubnima pomnilniškima
    ...
    r}) določi registrski par, katerega vrednost predstavlja pomnilniški naslov
    operanda ~\cite{strojnijezikzaprocesorz80} ~\cite{z80cpuusermanual}.

    Relativno naslavljanje v zlogu, ki sledi št. strojnega ukaza, vsebuje odmik v
    obliki dvojiškega komplementa, ki se prišteje vrednosti registra PC za
    določitev pomnilniške lokacije operanda. To poleg prenosljivosti programa omogoča
    tudi skoke na bližje pomnilniške lokacije z le dvozložnim ukazom ~\cite{z80cpuusermanual}.

    Indeksno naslavljanje (oblik \texttt{LD r, (IX+dis)} in \texttt{LD (IX+dis), r})
    je le oblika registskega posrednega naslavljanja z dodanim odmikom. V zlogu,
    ...
    uporabljeni podprogrami ROMa ~\cite{z80cpuusermanual}.

    Implicitno naslavljanje se uporablja pri mnogih ukazih, ki implicitno določijo
    registre z operandi, potrebnimi za dano operacijo (npr. rezultat
    aritmetičnih operacij se implicitno shrani v akumulator) ~\cite{z80cpuusermanual}.
    ...
    omogoči. Program lahko prav tako ukazom HALT prekine izvajanje strojnega programa
    do naslednje prekinitve. Med tem CPE izvaja operacije NOP za namen delovanja
    osveževanja pomnilnika ~\cite{strojnijezikzaprocesorz80} ~\cite{z80cpuusermanual}.

    \subsection{Načini prekinitev in njihovo delovanje}

    Z80 CPE podpira 2 vrsti prekinitev:

    Maksabilne prekinitve INT, ki jih je mogoče programsko onemogočiti, z nastavljanjem
    ...
    so prikazane v tabeli \ref{z80cpuusermanual_tab2} ~\cite{z80cpuusermanual}.
    ...
    maskabilnih prekinitev ~\cite{z80cpuusermanual}.
    ...
    je odziv razen lokacije klica identičen odzivu na nemaskabilne prekinitve,
    ki kliče podprogram na pomnilniškem naslovu \texttt{0x0066}. CPE tudi v tem
    ...
    Prekinitve NMI imajo višjo prioriteto kot INT in jih ni mogoče programsko
    onemogočiti, saj se uporablja za predvsem za takojšnje odzive na najpomembnejše
    signale, ki se morajo obravnavati s čim manj zakasnitve. CPE ga zato obravnava
    že po koncu izvajanja trenutnega ukaza. Pri tem se najprej IFF1 onemogoči (in
    njegova prejšnja vrednost shrani v IFF2), kar za čas izvajanja PSP prepreči navadne
    prekinitve, nato se vrednost registra PC shrani na sklad, izvajanje pa preusmeri
    na pomnilniški naslov \texttt{0x0066} ~\cite{z80cpuusermanual}.
    ...
    maskirnih prekinitev \texttt{IFF1} in \texttt{IFF2}, trenutni prekinitveni
    način, stanje \texttt{HALT}, ter funkcionalnosti razhroščevalnika zastavica teka,
    prekinitvene točke in števec izvedenih ukazov.
    ...
    zapiše na sklad, nato se onemogočita \texttt{IFF1} in \texttt{IFF2}. Nemaskirna
    ...
    emulatorja ne zaustavi. Pri navadni hitrosti primerja število izvedenih T-stanj
    z realnim časom izvajanja in po potrebi počaka z izvajanjem nadaljnjega ukaza,
    da se izvajanje približa frekvenci 3,5 MHz. V hitrem načinu tega čakanja ni,
    zato emulator izvaja ukaze tako hitro, kot dopušča gostiteljski sistem.
    ...
    *Str. 114 - Diagram razbiranja naslovov znakov na zaslonu v pomnilniški zaslonski
    ...
    več funkcij. Razporeditev tipk je prikazana na sliki
    \ref{softspectrum48_pic1} ~\cite{zxspectrumintroduction}.
    ...
    vnosa je prikazana na sliki \ref{zxspectrumbasicprogramming_pic2} ~\cite{zxspectrumbasicprogramming}.
    ...
    *Str. 101 - Skica skupin tipk in njihovih kodov* ~\cite{strojnijezikzaprocesorz80}
    ...
    ZX Spectrum za trajno hrambo programov uporablja navadne avdio kasete.
    Program, spremenljivke ali drug del pomnilnika lahko shranimo z ukazom SAVE,
    kasneje pa ga naložimo z ukazom LOAD. Ukaz VERIFY ne nalaga podatkov, temveč
    preveri, ali se podatki na kaseti ujemajo s podatki v pomnilniku. Ukaz RUN
    deluje podobno kot LOAD, le da program po nalaganju tudi zažene. ~\cite{zxspectrumintroduction}
    ~\cite{zxspectrumbasicprogramming}

    Pri teh ukazih lahko navedemo ime programa, dolgo največ 10 znakov. Če pri ukazih
    LOAD, VERIFY ali RUN ime pustimo prazno, Spectrum uporabi prvi ustrezen program,
    ki ga najde na kaseti. ~\cite{zxspectrumintroduction} ~\cite{zxspectrumbasicprogramming}

    \subsection{Formati}

    Kasetne formate je treba ločiti od posnetkov stanja, kot sta formata datotek
    SNA in Z80. Posnetek stanja hrani le stanje pomnilnika, registrov in drugih
    ...
    Vidni potek standardnega nalaganja s kasete je prikazan na sliki
    \ref{howtapeloadingword_pic1}.
    ...
    tipov blokov. Če je bil program shranjen z navadnimi ROMovimi rutinami,
    ...
    Če se datoteka začne s podpisom \texttt{ZXTape!}, jo obravnava kot TZX, sicer
    kot TAP. Pri formatu TAP zaporedno bere dolžino bloka in surove podatke
    bloka. Pri formatu TZX prepozna standardne, turbo in čiste podatkovne bloke, druge
    podprte bloke pa preskoči, kadar ne vplivajo na podatke, ki jih trenutna implementacija
    potrebuje za nalaganje.
    ...
    podatke in kontrolno vsoto. Kontrolna vsota se preveri z operacijo XOR čez
    zastavični zlog in vse podatkovne zloge. Parser pri veljavnih standardnih zapisih
    označi tudi povezavo med blokom glave in naslednjim podatkovnim blokom, kar
    kasneje poenostavi izbiro pravega bloka pri zaporednem nalaganju.

    Objekt \texttt{ULATapeDeck} hrani trenutno vstavljeno kaseto, položaj bralne
    glave in morebitni podatkovni blok, ki pripada že prebrani glavi. Ko program
    v emuliranem Spectrumu pokliče ROM rutino \texttt{LD-BYTES}, procesor to
    zazna na naslovu \texttt{0x0556} in klic preda kasetnemu podsistemu. Ta iz
    registrov prebere pričakovani tip bloka, dolžino in ciljni naslov. Pri ukazu
    \texttt{LOAD} se koristni podatki iz izbranega bloka zapišejo v pomnilnik,
    pri \texttt{VERIFY} pa se enak obseg samo prebere in primerja z vsebino bloka.
    Po koncu se registri nastavijo tako, kot bi se po vrnitvi iz ROM rutine,
    nato se izvajanje nadaljuje pri skupnem izhodu iz nalagalne rutine.

    Tak pristop ne emulira zvočnih impulzov kasete v realnem času, temveč posnema
    učinek standardnih ROM rutin za nalaganje. Prednost je bistveno hitrejše in
    zanesljivejše nalaganje programov v razhroščevalniku. Omejitev pa je, da
    ...
    Kode tonov za ukaz \texttt{BEEP} so prikazane na sliki
    \ref{zxspectrumbasicprogramming_pic3}.
    ...
    zvočni signal ne pride le do zvočnika, temveč istočasno tudi do MIC in EAR
    vtičnic. ~\cite{zxspectrumbasicprogramming}

    Na zvočnik lahko poleg z ukazom BEEP prek ROMa vplivamo tudi z ukazi OUT skozi
    V/I vrata \texttt{0xFE}. Bita 4 v zlogu na vratih določa stanje zvočnika (0
    pomeni izključen, 1 pomeni vključen) - frekvenco pa določamo s hitrostjo
    menjav med stanjema. ~\cite{strojnijezikzaprocesorz80}

    \subsection{Implementacija v emulatorju}

    Zvočni del je vezan na pisanje v V/I vrata z nižjim zlogom \texttt{0xFE}.
    Vsa pisanja v V/I vrata gredo skozi razred \texttt{IOPortSet}; ta shrani
    zapisano vrednost in jo posreduje objektu \texttt{ULABeeper}. Zvočnik
    uporablja bit 4 zapisane vrednosti, zato se frekvenca zvoka v praksi določi s
    hitrostjo, s katero program menja ta bit.

    \texttt{ULABeeper} je v skupni kodi definiran kot pričakovani objekt \texttt{expect},
    ker je dejansko predvajanje zvoka odvisno od platforme. Na JVM je
    implementiran z zvočno linijo \texttt{SourceDataLine}. Ob vsakem zapisu na
    vrata \texttt{0xFE} primerja trenutno število T-stanj s časom prejšnjega zapisa
    in iz razlike izračuna, koliko vzorcev mora zapisati v zvočni izhod. Kadar je
    bit 4 nastavljen, zapisuje vzorce z visoko amplitudo, sicer tišino. S tem se
    signal zvočnika gradi iz istega časovnega modela, ki ga uporablja procesor.

    V spletnih ciljnih platformah je trenutno implementirano le stanje izklopa zvoka,
    samo predvajanje pa je prazna operacija. To pomeni, da je zvočna emulacija
    funkcionalna v namizni različici, v spletni različici pa uporabniški vmesnik ohrani
    enake kontrole, vendar zvočni izhod še ni izveden.
    ...
    contention - Port \texttt{0xFE} (ULA port) contention based on address bus high
    byte - Snow effect

    Raje bi uporabil kaj drugega kot KMP, ker cross-platform funkcionalnosti po mojem
    mnenju niso uporabne za razhroščevalnik (kar je glavna komponenta tega
    projekta), prav tako je KMP za desktop privzeto bolj omejen tako v
    funkcionalnostih kot preformanceu (moral iti nivo nižje od KMP za kaj takega).
    Tako bi razmišljal o drugih frameworkih, kot so Electron + TS ali Dioxus/Tauri
    + Rust.
