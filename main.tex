...
\begin{document}
    ...
    \begin{tabular}{p{0.12\textwidth}|p{.37\textwidth}|p{.43\textwidth}}
        ...
        {\bf INT}     & interrupt                                          & maskirna prekinitev                                        \\
        ...
        {\bf NMI}     & non-maskable interrupt                             & nemaskirna prekinitev                                      \\
        ...
        {\bf TZX}     & ZX tape                                            & zapis kasete s časovnimi podatki                           \\
    ...
    sistema ZX Spectrum 48K. Emulator mora podpirati poganjanje programov napisanih
    za Spectrum in razhroščevanje z vpogledom v notranje stanje sistema. Zaradi
    poudarka na razhroščevanje mora pri tem omogočati pregled in spreminjanje
    vrednosti registrov ter pomnilnika, pregled in razlago pomena naloženih ukazov
    in delov pomnilnika. Tak vpogled ni pomemben le za boljše razumevanje
    sistema, temveč tudi ohranjanje znanja o njegovem delovanju. Delovanje
    ...
    njegov vpliv na slovensko in svetovno okolje, in pomen ohranjanja takega znanja.
    ...
    prikazana na sliki \ref{i4cysinclairomputers_pic1}. Bila sta bila namenjena predvsem
    ...
    sta uporabljali procesor Zilog Z80 in 16 KB ROMa. Računalnik se je priključil
    ...
    (primer delovanja prikazan na sliki \ref{subspectrum_ines}), slik ter
    ...
    prikazane na sliki \ref{zxspectrumintroduction_pic1} ~\cite{zxspectrumintroduction}.
    ...
    Razporeditev glavnih notranjih komponent računalnika je prikazana na sliki
    ...
    prikazuje tudi notranje stanje emuliranega računalnika. Zato posamezni deli sistema
    niso skriti za emulacijsko zanko, ampak so implementirani kot ločeni modeli
    komponent delujočega sistema: registri, pomnilnik, V/I vrata, časovnik ter
    ostale naprave in funkcionalnosti računalnika. Procesor vrednosti stanja teh modelov
    spreminja med izvajanjem ukazov, uporabniški vmesnik pa jih opazuje in
    ...
    Podokno za registre prikazuje vrednosti splošnin in posebnih registrov ter
    ...
    ponavljajo zaustavitve ali prekinitvene točke, preklopi hitrost izvajanja emulacije,
    ...
    \begin{itemize}
        ...
        \item CHARS na naslovu \texttt{0x5C36}, ki vsebuje naslov na začetek nabora
            znakov.
    \end{itemize}
    ...
    anotiran pogled. Stalna območja RAMa, kot so ROM, zaslonska datoteka,
    ...
    20 Gorenje, Amstrad CPC 454 in celo večji računalniki, kot je Iskrin Partner ~\cite{strojnijezikzaprocesorz80}.
    ...
    Glavni notranji elementi procesorja so prikazani na sliki
    ...
    Nekatere operacije pa so 16-bitne, torej CPE dela z vrednosti med \texttt{0x0000}
    ...
    registrski petorček AFBCDE, bodisi računski sklad v pomnilniku. Vender pa
    tak tip podatka ni specifičen za procesor Z80, temveč za podprograme ROMa računalnika
    ZX Spectrum ~\cite{strojnijezikzaprocesorz80}.
    ...
    strani prekinitev I. Razporeditev notranjih registrov je prikazana na sliki
    ...
    Med osnovnim in zamenljivim registrskim naborom (keterega imena registrov označujemo
    z apostrofom ob imenih) se lahko menjava. Ta značilnost procesorja Z80 je posebej
    potrebna v sistemih, ki zahtevajo hiter odziv na prekinitve. Ob pričetku obdelave
    prekinitev ali izvajanja podprograma je potrebno star izvajalni kontekst (vrednosti
    registrov) shraniti do obnovitve pred vrnitvijo iz podprograma. Zamenljiva
    registrska nabora tako omogočata mnogo hitrejšo menjavo konteksta napram
    hrambi in obnavaljanju vrednosti vseh registrov v pomnilniku ~\cite{strojnijezikzaprocesorz80}
    ~\cite{z80cpuusermanual}.
    ...
    programa, ki se dopolnejo z nižjim zlogom naslova, ki jih zagotovi zunanja
    naprava, ki je sprožila prekinitev. ko naprava, ki povzroči prekinitev.
    Procesor Z80 namreč deluje v načinu, v katerem je mogoče v odziv na prekinitev
    izvesti posredni klic na PSP na poljubnem mestu v pomnilniku ~\cite{z80cpuusermanual}.

    Register osveževanja pomnilnika (R) hrani 7-bitni števec osveževanja
    pomnilnika, ki se samodejno poveča po vsakem pridobljenem ukazu, osmi bit se
    uporablja le pri ukazu \texttt{LD R, A}. To omogoča enostavno uporabo
    dinamičnih in statičnih pomnilnikov brez upočasnjevanja delovanja CPE (osveževanje
    se dogaja med dekodiranjem prej pridobljenega ukaza) ~\cite{z80cpuusermanual}.
    ...
    pa višjega ~\cite{strojnijezikzaprocesorz80} ~\cite{z80cpuusermanual}.
    ...
    naslov hrani na skladu v pomnilniku ~\cite{strojnijezikzaprocesorz80} ~\cite{z80cpuusermanual}.
    ...
    da ima register F možnost shranjevanja 8 bitov, za dokumentirane zastavice uporablja
    le 6 bitov (bita 3 in 5 se ne uporabljata) ~\cite{strojnijezikzaprocesorz80}
    ~\cite{z80cpuusermanual}:
    ...
    \begin{itemize}
        ...
        \item zastavica parnosti/prepolnjenja P/V (angl. Parity/oVerflow flag)
            ...
            ali ko se je spremenil 8. bit registra (kar pomeni možnost preliva),
    ...
    prilagajanju katerega koli BASIC programa za delovanje na ZX Spectrum ~\cite{zxspectrumintroduction}.
    ...
    razširjen zbirnik za uporabo v Spectrumu je bil program GENS ~\cite{zxspectrumbasicprogramming}
    ~\cite{strojnijezikzaprocesorz80}.

    \subsection{Število ukazov in sklopi, njihove dolžine}

    Procesor Z80 lahko izvaja 158 različnih (dokumentiranih) ukazov, vključno z vsemi
    78 ukazi procesorja 8080A ~\cite{z80cpuusermanual}.
    ...
    *Str. 132-133 - kodi operacij za ukaz RST 28 in tabele podprogramov* ~\cite{strojnijezikzaprocesorz80}
    ...
    operanda ali skočne lokacije, kar omogoča prenašanje vrednosti iz registra v
    pomnilniško celico in obratno ter skok ali prenos podatkov med poljubnima
    pomnilniškima lokacijama ~\cite{strojnijezikzaprocesorz80} ~\cite{z80cpuusermanual}.
    ...
    skoke na bližje pomnilniške lokacije z le dvozložnim ukazom ~\cite{z80cpuusermanual}.

    Indeksno naslavljanje (oblik \texttt{LD r, (IX+dis)} in \texttt{LD (IX+dis), r})
    je le oblika registskega posrednega naslavljanja z dodanim odmikom. V zlogu, ki
    sledi št. strojnega ukaza, vsebuje odmik v predstavitvi dvojiškega komplementa,
    ...
    Bitno naslavljanje s pomočjo registrskega, registskega posrednega ali
    ...
    ukazih so prikazane v tabeli \ref{z80cpuusermanual_tab2} ~\cite{z80cpuusermanual}.
    ...
    \begin{table}[htb]
        \begin{center}
            \begin{tabular}{|p{0.28\textwidth}|c|c|p{0.42\textwidth}|}
                ...
                \hline
                Izvedba ukaza \texttt{RETN}   & IFF2          & *             & Dvignjena IFF2 označi konec rutine za obdelavo nemaskirne prekinitve. \\
                \hline
            \end{tabular}
        \end{center}
    ...
    registrov, pomnilnika, V/I vrat in časovnika. V objektu \texttt{Processor}
    ...
    stanje \texttt{HALT}, ter funkcionalnosti razhroščevalnika zastavica teka, prekinitvene
    točke in števec izvedenih ukazov.
    ...
    metodami za branje in nastavljanje posameznih bitov.

    Posamezen strojni ukaz je predstavljen z vmesnikom \texttt{Instruction}.
    ...
    Opis ukaza je ločen od njegove izvedbe z vmesnikom \texttt{InstructionDefinition}.
    Definicija vsebuje bitni vzorec ukaznega zloga oziroma zaporedja zlogov in
    metodo za dekodiranje operandov.
    ...
    prišteje število T-stanj ukaza. Pogoji za izvedbo maskirne prekinitve se preverjajo
    ob koncu izvajanja trenutnega ukaza.
    ...
    Nemaskirna prekinitev ponastavi vrednost le \texttt{IFF1}, na sklad shrani
    ...
    ne zaustavi. Pri navadni hitrosti primerja število izvedenih T-stanj z
    realnim časom izvajanja in po potrebi počaka z izvajanjem nadaljnjega ukaza, da
    se izvajanje približa frekvenci 3,5 MHz. V hitrem načinu tega čakanja ni, zato
    emulator izvaja ukaze tako hitro, kot dopušča gostiteljski sistem.

    Pravilnost implementacije ukazov je bila preverjana tudi s programoma
    \texttt{zexdoc} in \texttt{zexall}, različicama preizkuševalnika ukazov Z80,
    prilagojenima za ZX Spectrum. \texttt{zexdoc} preverja dokumentirane
    zastavice, registre in pomnilnik, \texttt{zexall} pa poleg tega preverja tudi
    nedokumentirani zastavici XF in YF. Programa izvajata skupine ukazov z
    različnimi začetnimi stanji, iz končnega stanja izračunata kontrolno vsoto CRC
    in jo primerjata z referenčno vrednostjo, pridobljeno na pravem procesorju
    Z80. Preizkus ne pove, katera podrobnost ukaza je napačna, temveč samo, da
    se rezultat razlikuje od referenčnega izvajanja ~\cite{z80exerciseravery} ~\cite{spectrumz80exerciser}.
    ...
    obdeluje shranjene podatke. Na popolnoma enak način dela tudi s 65536 V/I naslovi,
    prek katerih komunicira z zunanjimi napravami (tipkovnica, zvočnik, barvo obrobe
    zaslona, kasetofon prek vtičnic EAR in MIC, tiskalnik in drugi robni vmesniki
    in naprave pa prek robnega priključka). ~\cite{zxspectrumbasicprogramming} ~\cite{strojnijezikzaprocesorz80}

    V/I naslovi je sestavljeni iz 16 bitov (A15, A14, ..., A2, A1, A0), ki imajo
    različne pomene - vrednost 0 katerikoli bita od A0, A1, A2, A3 ali A4 predstavlja
    indikator specifične operacije, bitov A6 in A7 računalnik ne uporablja, bita A8
    in A9 včash podajata dodatne (nedokumentirane) informacije.

    Ker v večini primerov višji zlog V/I naslova nima nobenega vpliva na delovanje,
    je odločilen le nižji zlog V/I naslova, ki označuje V/I vrata (različni V/I
    naslovi tako mnogokrat naslavljajo ista izmed 256 V/I vrat). ~\cite{zxspectrumbasicprogramming}
    ~\cite{strojnijezikzaprocesorz80}
    ...
    Vrednosti na V/I naslovih, ki prehajajo med zunanjimi napravami prek V/I vrat
    in CPE so sestavljena sestavljena iz 8 bitov ~\cite{zxspectrumbasicprogramming}
    ~\cite{strojnijezikzaprocesorz80}

    Kontrolno, podatkovno in naslovno vodilo so (med drugim) dostopna na zadnji strani
    računalnika. *Str. 118 - prikaz dostopnih povezav do vodil* ~\cite{zxspectrumbasicprogramming}
    ...
    Časovni model je implementiran v objektu \texttt{ULATiming}. Ta hrani skupno število
    izvedenih T-stanj, trenutno pozicijo znotraj slikovnega okvira in števec
    prikazanih okvirjev. Emulator uporablja frekvenco procesorja 3,5 MHz in dolžino
    okvirja 69888 T-stanj, kar ustreza 48K modelu z evropskim video standardom.

    Po vsakem izvedenem ukazu procesor časovniku prišteje trajanje ukaza. Ko število
    T-stanj preseže dolžino okvirja, časovnik poveča števec okvirjev, označi čakajočo
    prekinitev in o tem obvesti dele uporabniškega vmesnika, ki so vezani na
    osveževanje slike. Procesor nato po koncu ukaza preveri, ali je prekinitev
    omogočena, in jo obdela. Časovnik zato povezuje hitrost izvajanja, periodične
    prekinitve in osveževanje zaslonskega prikaza.
    ...
    Zaslonska slika je razdeljena na 768 položajev (prek 24 vrstic in 32 stolpcev),
    ...
    *Str. 114 - Diagram razbiranja naslovov PRINT položajev in njihove naslove
    ...
    Nabor grafičnih simbolov je prikazan na sliki
    \ref{zxspectrumbasicprogramming_pic1}.
    ...
    \begin{figure}[htb]
        ...
        \end{center}
        \caption{Tabela grafičnih simbolov ~\cite{zxspectrumbasicprogramming}.}
        \label{zxspectrumbasicprogramming_pic1}
    \end{figure}

    V ROMu so med naslovoma 3D00 in 3FFF shranjeni točkovni vzorci vseh znakov razen
    grafičnih. Naslov posameznega znaka, ki ima kod X (npr. 'a' ima kod 61H)
    izračunamo po formuli $8x+3C00$ ~\cite{strojnijezikzaprocesorz80}

    Povezava med simboli na tipkah in načini vnosa je prikazana na sliki
    \ref{zxspectrumbasicprogramming_pic2}.
    ...
    Običajno, ko je nekaj zapisano na položaj znaka, popolnoma uniči tisto, kar
    je bilo tam zapisano prej. Lahko pa spremembo naredimo tako, da bo novi znak
    preprosto dodan na vrhu starega. To je lahko še posebej koristno za pisanje sestavljenih
    znakov, kot so črke s poudarki na njih, kot v tem programu za tiskanje nemških
    črk - "o" z umlaut nad njim. ~\cite{zxspectrumbasicprogramming}
    ...
    ZX Spectrum lahko na (barvnem) zaslonu prikazuje osem barv, označene s
    številkami od 0 do 7:
    \begin{itemize}
        \item 0 - črna
    ...
    zaslonih, kjer se barve v danem vrstnem redu prikažejo v padajočih odtenkih sivin.
    Drugi razlog se opira na dejstvo, da lahko človeško oko resnično vidi le tri primarne
    barve (modro, rdečo in zeleno), druge barve pa so mešanice le-teh. Tak
    vrstni red barv omogoča "mešanje" barv s seštevanjem kod primarnih barv -
    npr. magenta je narejena z mešanjem modre in rdeče barve, zato je njena koda
    (3) vsota kod za modro (1) in rdečo (2). ~\cite{zxspectrumintroduction} ~\cite{zxspectrumbasicprogramming}
    ...
    Ker se atributi nanašajo na vseh 64 pik položaja znaka (ne pa na posamezne
    pike), ne moremo določiti več kot 2 barv ali nastavitev svetlosti in
    utripanja. Ko nekaj natisnemo na zaslon, poleg vzorca pik spremenimo tudi
    nastavitve atributov položaja. ~\cite{zxspectrumbasicprogramming}
    ...
    končne naslove zaslonske datoteke ter datoteke atributov. V njem je tudi preslikava
    Spectrumovih barv v barve uporabniškega vmesnika ter funkcije za razlago bitov
    atributnega zloga.

    Okno zaslona ob vsakem novem okviru iz pomnilnika prebere območje \texttt{0x4000}-\texttt{0x57FF}
    in območje \texttt{0x5800}-\texttt{0x5AFF}. Prvo območje določa vzorec pik, drugo
    pa barve črnila, papirja, svetlost in utripanje za posamezen položaj znaka.
    Funkcija za izračun indeksa zloga zaslonske datoteke upošteva ne-linearen
    razpored vrstic, ki ga uporablja ZX Spectrum. Prikaz je narisan na platno
    Compose, pri čemer se velikost navidezne pike prilagodi velikosti okna, da
    ostane razmerje slike nespremenjeno.
    ...
    Razporeditev tipk je prikazana na sliki \ref{softspectrum48_pic1}.
    ...
    tipk moramo pri pritisku uporabiti tipko CAPS SHIFT. Stranske puščice premikajo
    kurzor v levo ali desno, tipka DELETE pa izbriše znak neposredno pred
    kurzorjem. ~\cite{zxspectrumintroduction}

    Ko pritisnete ENTER, bo računalnik vaše sporočilo zapisal na vrh zaslona -
    ali pod tisto, ki ga je zapisal prvič, če je še vedno tam. ~\cite{zxspectrumintroduction}

    Če katero koli tipko pridržimo pritisnjeno več kot tri sekunde, se ta
    samodejno ponovi, tipka pa takrat oddaja tiho klikajoč zvok. ~\cite{zxspectrumintroduction}
    ~\cite{zxspectrumbasicprogramming}
    ...
    tipke pet ali več različnih pomenov, do katerih pridemo bodisi prek SHIFT tipk
    (CAPS SHIFT ali SYMBOL SHIFT) bodisi z menjavanjem načinov delovanja tipkovnice.
    ~\cite{zxspectrumbasicprogramming}

    \subsection{Načini delovanja}

    Kurzor je predstavljen kot utripajoča črka, ki označuje tako položaj izpisa
    naslednjega vnešenega znaka kot tudi trenutni način delovanja tipkovnice
    prek simbola kurzorja. ~\cite{zxspectrumintroduction} ~\cite{zxspectrumbasicprogramming}
    ...
    KEY SCAN (naslov \texttt{0x02BE}) vrne rezultat v registrih D in E:
    ...
    *Str. 121 - Skica tipkovnice Spectruma in kodi tipk (razvrščeni v spirali v nasprotni
    ...
    programi raje poslužijo zgoraj omenjenega podprograma iz ROMa. ~\cite{strojnijezikzaprocesorz80}

    \subsection{Implementacija v emulatorju}

    Tipkovnica je v emulatorju implementirana kot matrika osmih pol-vrstic s po
    petimi biti. Nepritisnjena tipka ima vrednost 1, pritisnjena pa vrednost 0, enako
    kot pri branju tipkovnice prek V/I vrat na pravem Spectrumu. Objekt \texttt{ULAKeyboard}
    hrani trenutno stanje matrike in pri branju V/I naslova z nižjim zlogom \texttt{0xFE}
    preveri, katere vrstice so izbrane z višjim zlogom naslova. Rezultat je
    bitni AND izbranih vrstic, dopolnjen z višjimi biti, ki niso vezani na
    posamezne tipke.

    Vhod iz gostiteljskega sistema se najprej prevede v eno ali več tipk matrike.
    Emulator podpira dva načina: pristni način, kjer fizične tipke približno ustrezajo
    tipkam Spectruma, in način gostiteljske razporeditve, kjer se znak, ki ga
    pošlje operacijski sistem, preslika v ustrezno kombinacijo tipk Spectruma.
    Pri velikih črkah in znakih, ki jih Spectrum vnese s tipko \texttt{SYMBOL
    SHIFT}, se zato v matriki hkrati nastavita tipka za zamik in osnovna tipka znaka.
    Ob spremembi načina se vse tipke sprostijo, da v emuliranem računalniku ne ostane
    navidezno pritisnjena tipka.
    ...
    dva zloga dolžine, nato pa sledijo zastavični zlog, zlog s kontrolno vsoto,
    in dejanski podatki. Format je zato preprost in primeren za programe,
    ...
    Vidni potek standardnega nalaganja s kasete je prikazan na sliki \ref{howtapeloadingword_pic1}.
    ...
    za uporabo v emulatorjih. Datoteka \texttt{.tzx} se začne z glavo \texttt{ZXTape!}
    in številko različice, za tem pa sledi en ali več različnih tipov blokov. Če
    je bil program shranjen z navadnimi ROMovimi rutinami, datoteka v formatu TZX
    načeloma vsebuje enake standardne podatkovne bloke kot TAP. Njegova prednost
    je, da lahko poleg osnovnih tipov blokov hrani tudi tudi pavze, časovne
    podatke o impulzih in druge posebne bloke, ki jih uporabljajo posebni nalagalniki
    ali zaščite. Zato je TZX primernejši takrat, ko želimo v emulatorju čim bolj natančno
    ponoviti potek izvirnega nalaganja s kasete. ~\cite{tzxformat} ~\cite{worldofspectrumformats}
    ...
    Če se datoteka začne s podpisom \texttt{ZXTape!}, jo obravnava kot TZX,
    sicer kot TAP. Pri formatu TAP zaporedno bere dolžino bloka in surove podatke
    bloka. Pri formatu TZX prepozna standardne, turbo in čiste podatkovne bloke,
    druge podprte bloke pa preskoči, kadar ne vplivajo na podatke, ki jih
    trenutna implementacija potrebuje za nalaganje.
    ...
    čez zastavični zlog in vse podatkovne zloge. Parser pri veljavnih
    standardnih zapisih označi tudi povezavo med blokom glave in naslednjim
    podatkovnim blokom, kar kasneje poenostavi izbiro pravega bloka pri zaporednem
    nalaganju.

    Objekt \texttt{ULATapeDeck} hrani trenutno vstavljeno kaseto, položaj bralne glave
    in morebitni podatkovni blok, ki pripada že prebrani glavi. Ko program v emuliranem
    Spectrumu pokliče ROM rutino \texttt{LD-BYTES}, procesor to zazna na naslovu \texttt{0x0556}
    in klic preda kasetnemu podsistemu. Ta iz registrov prebere pričakovani tip bloka,
    dolžino in ciljni naslov. Pri ukazu \texttt{LOAD} se koristni podatki iz
    izbranega bloka zapišejo v pomnilnik, pri \texttt{VERIFY} pa se enak obseg
    samo prebere in primerja z vsebino bloka. Po koncu se registri nastavijo tako,
    kot bi se po vrnitvi iz ROM rutine, nato se izvajanje nadaljuje pri skupnem izhodu
    iz nalagalne rutine.

    Tak pristop ne emulira zvočnih impulzov kasete v realnem času, temveč
    posnema učinek standardnih ROM rutin za nalaganje. Prednost je bistveno hitrejše
    in zanesljivejše nalaganje programov v razhroščevalniku. Omejitev pa je, da posebni
    nalagalniki, ki so odvisni od natančnega trajanja impulzov ali od
    neposrednega branja signala EAR, niso polno podprti.
    ...
    Kode tonov za ukaz \texttt{BEEP} so prikazane na sliki \ref{zxspectrumbasicprogramming_pic3}.
    ...
    in EAR vtičnic. ~\cite{zxspectrumbasicprogramming}

    Na zvočnik lahko poleg z ukazom BEEP prek ROMa vplivamo tudi z ukazi OUT
    skozi V/I vrata \texttt{0xFE}. Bita 4 v zlogu na vratih določa stanje
    zvočnika (0 pomeni izključen, 1 pomeni vključen) - frekvenco pa določamo s hitrostjo
    menjav med stanjema. ~\cite{strojnijezikzaprocesorz80}

    \subsection{Implementacija v emulatorju}

    Zvočni del je vezan na pisanje v V/I vrata z nižjim zlogom \texttt{0xFE}. Vsa
    pisanja v V/I vrata gredo skozi razred \texttt{IOPortSet}; ta shrani zapisano
    vrednost in jo posreduje objektu \texttt{ULABeeper}. Zvočnik uporablja bit 4 zapisane
    vrednosti, zato se frekvenca zvoka v praksi določi s hitrostjo, s katero program
    menja ta bit.

    \texttt{ULABeeper} je v skupni kodi definiran kot pričakovani objekt \texttt{expect},
    ker je dejansko predvajanje zvoka odvisno od platforme. Na JVM je implementiran
    z zvočno linijo \texttt{SourceDataLine}. Ob vsakem zapisu na vrata \texttt{0xFE}
    primerja trenutno število T-stanj s časom prejšnjega zapisa in iz razlike
    izračuna, koliko vzorcev mora zapisati v zvočni izhod. Kadar je bit 4
    nastavljen, zapisuje vzorce z visoko amplitudo, sicer tišino. S tem se signal
    zvočnika gradi iz istega časovnega modela, ki ga uporablja procesor.

    V spletnih ciljnih platformah je trenutno implementirano le stanje izklopa
    zvoka, samo predvajanje pa je prazna operacija. To pomeni, da je zvočna emulacija
    funkcionalna v namizni različici, v spletni različici pa uporabniški vmesnik
    ohrani enake kontrole, vendar zvočni izhod še ni izveden.
